...
@section('content')
    ...
    <div class="app-content">
        <div class="container-fluid">
            ...
            <x-table-component :headers="['#', 'Image','Service','Name', 'Description', 'Status', 'Action']">
                <x-slot name="actions">
                ...
                @foreach ($brandPortfolios as $item)
                    <tr style="padding: 0" data-id="{{ $item->id }}">
                        ...
                        <td style="padding: 0">{{ $item?->service?->name }}</td>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="branding_service_id" id="" class="form-control form-select">
                                ...
                                @foreach ($brandServices as $pItem)
                                    <option value="{{ $pItem?->id }}" {{$brandPortfolio?->branding_service_id == $pItem->id ? 'selected' : ''}}>{{ $pItem?->name }}</option>
                                @endforeach
                            </select>
                            @error('name')
