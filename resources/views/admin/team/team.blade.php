...
@section('content')
    ...
    <div class="app-content">
        <div class="container-fluid">
            ...
            <x-table-component :headers="['#', 'Image', 'Name','Position', 'Action']">
                <x-slot name="actions">
                ...
                @foreach ($teams as $item)
                    <tr style="padding: 0" data-id="{{ $item->id }}">
                        ...
                        <td style="padding: 0">
                            ...
                            @else
                                <div class="d-flex justify-content-center align-items-center" style="background-color: #fff;height:70px;width:70px;border-radius:50%">
                                    <span>{{getName($item->name)}}</span>
                                </div>
                            @endif
                        </td>
                        <td style="padding: 0">{{ $item->name }}</td>
                        <td style="padding: 0">{{ $item->position }}</td>
                       
                        <td style="padding: 0">
                            <div class="d-flex">
                                <a href="{{ route('team.edit', $item) }}" style="margin-top: 10px"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ $team?->id ? 'Update' : 'Create' }} team member
                    </h5>
                </div>
                <form
                    action="{{ $team?->id ? route('team.update', $team) : route('team.store') }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex align-items-end">
                                <div class="">
                                    @if ($team->id)
                                        <img id="preview" src="/storage/{{ $team->image }}" alt="Preview"
                                            class="img-thumbnail"
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit"
                            class="btn btn-primary">{{ $team?->id ? 'Update' : 'Save' }}</button>
                    </div>
                </form>
            </div>
        </div>
