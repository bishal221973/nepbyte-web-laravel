...
@section('content')
    <div class="app-content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="mb-0">Print Category</h3>
                </div>
                <div class="col-sm-6">
            ...
            <x-table-component :headers="['#', 'Name', 'Status', 'Action']">
                ...
                @foreach ($printCategories as $item)
                    <tr style="padding: 0" data-id="{{ $item->id }}">
                        ...
                        <td style="padding: 0">
                            <div class="d-flex">
                                <a href="{{ route('print-category.edit', $item) }}" style="margin-top: 10px"
                                    class="btn text-warning shadow-none">Edit</a>

                                <form action="{{ route('print-category.destroy', $item) }}" method="POST"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ $printCategory?->id ? 'Update' : 'Create' }} Category
                    </h5>
                </div>
                <form
                    action="{{ $printCategory?->id ? route('print-category.update', $printCategory) : route('print-category.store') }}"
                    method="POST" enctype="multipart/form-data">
                    @csrf
                    @isset($printCategory?->id)
                        @method('PUT')
                    @endisset
                    <div class="modal-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" placeholder="Category" name="title"
                                value="{{ old('title', $printCategory?->title) }}">
                            @error('title')
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit"
                            class="btn btn-primary">{{ $printCategory?->id ? 'Update' : 'Save' }}</button>
                    </div>
                </form>
            </div>
        </div>
